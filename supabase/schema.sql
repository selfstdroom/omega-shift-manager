...
create table public.projects (id text primary key default gen_random_uuid()::text, company_id text not null references public.companies(id) on delete cascade, workplace_id text not null references public.workplaces(id) on delete restrict, title text not null, work_date date not null, start_time time not null, end_time time not null, location text not null default '', required_people int not null check(required_people>0), required_leaders int not null default 1 check(required_leaders>=0), note text not null default '', created_at timestamptz not null default now(), check(start_time<end_time));
...
create table public.assignments (id text primary key default gen_random_uuid()::text, company_id text not null references public.companies(id) on delete cascade, project_id text not null references public.projects(id) on delete cascade, staff_id text not null references public.profiles(id) on delete cascade, run_id text not null references public.assignment_runs(id) on delete cascade, status public.assignment_status not null default 'draft', is_leader boolean not null default false, created_at timestamptz not null default now(), unique(run_id,project_id,staff_id));
...
alter table public.companies enable row level security; alter table public.workplaces enable row level security; alter table public.admin_accounts enable row level security; alter table public.profiles enable row level security; alter table public.project_templates enable row level security; alter table public.projects enable row level security; alter table public.availability_periods enable row level security; alter table public.availabilities enable row level security; alter table public.assignment_runs enable row level security; alter table public.assignments enable row level security; alter table public.notifications enable row level security;
